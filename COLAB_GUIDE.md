...
1. Tại thư mục dự án trên máy tính, bạn chỉ cần nhấp đúp vào file:
   👉 [`tao_file_zip_colab.bat`](file:///e:/Tools/TSS-main/tao_file_zip_colab.bat) (hoặc chạy lệnh `python package_colab_zip.py`).
2. Script sẽ tự động gom toàn bộ mã nguồn `src/`, `webui/`, `HD.md`, cấu hình... thành 1 file duy nhất:
   👉 **`TSS_Code.zip`** (Dung lượng siêu nhẹ ~7.5 MB).
...
2. Mở [Google Colab](https://colab.research.google.com) và tải lên file [`ZeroTTS_Colab_FreeTier.ipynb`](file:///e:/Tools/TSS-main/ZeroTTS_Colab_FreeTier.ipynb).
3. Bật GPU T4: Vào **Runtime** -> **Change runtime type** -> Chọn **T4 GPU** -> Nhấn **Save**.
4. Chọn **Runtime** -> **Run all (Chạy tất cả)**:
   - Colab sẽ tự nhận diện file `TSS_Code.zip` từ Google Drive, giải nén toàn bộ tính năng và cài đặt môi trường.
